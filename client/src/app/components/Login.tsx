...
const Login = ({ title, fields }: LoginProps) => {
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-50 dark:bg-gray-900 px-4">
      <div className="w-full max-w-md p-8 bg-white dark:bg-gray-800 rounded-xl shadow-lg">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
          {fields.map((field) => (
            <div key={field.name} className="flex flex-col relative">
              <label className="text-gray-700 dark:text-gray-200 mb-1">
                {field.label}
              </label>
              <input
                {...register(field.name, {
        ...
        <button
          ...
          className="flex items-center justify-center w-full border border-gray-300 dark:border-gray-600 py-2 rounded-md gap-2 hover:bg-gray-100 active:scale-[0.95] dark:hover:bg-gray-700 transition duration-300 delay-75 cursor-pointer text-black dark:text-white"
        >
